...
export default class ThreeDMux extends Template {
  ...
  __drawFlow(params) {
    ...
    let bottomlineleft = new paper.Point(
      px,
      py - channelWidth / 2 + vertlinelength
    );
    let bottomlineright = new paper.Point(
      px + bottomlinelength,
      py + channelWidth / 2 + vertlinelength
    );
    ...
    for (var i = 0; i < N; i++) {
      ...
      let vertlinebottom = new paper.Point(
    ...
    // create output port
    let portCon = new paper.Point(
      px + bottomlinelength / 2 - channelWidth / 2,
      py + vertlinelength
    );
    let portOut = new paper.Point(
      px + bottomlinelength / 2 + channelWidth / 2,
    ...
    for (var j = 0; j < valvenum; j++) {
      ...
      while (count2 < N) {
        for (var w = 0; w < cur_N / 2; w++) {
          ...
          branchArray[N - 1 - w - count2] = branchArray[
            N - 1 - w - count2
          ].subtract(cutrec);
          this.__createthreedmuxValve(
            threedmux_flow,
  ...
  __drawControl(params) {
    ...
    for (var i = 0; i < 2 * valvenum; i++) {
      ...
      if (i % 2 === 0) {
        indexN /= 2;
        let cur_ind = N - indexN - 1;
        let leftsideLeft = new paper.Point(
          leftInput,
          py +
            vertholder +
        ...
        let leftsideRight = new paper.Point(
        ...
        let center = new paper.Point(
          px + cur_ind * (bottomlinelength / (N - 1)),
          py + vertholder + (i * vertlinelength) / (2 * valvenum + 2)
        );
        let circle = new paper.Path.Circle(center, radius);
        threedmux_control.addChild(circle);
      ...
      //right side
        ...
        let cur_ind = indexN;
        let rightsideLeft = new paper.Point(
          px + cur_ind * (bottomlinelength / (N - 1)),
          py +
            vertholder +
            (i * vertlinelength) / (2 * valvenum + 2) -
        ...
        let circle = new paper.Path.Circle(center, radius);
        threedmux_control.addChild(circle);
        let rightsideRight = new paper.Point(
          rightInput,
          py +
            vertholder +
            (i * vertlinelength) / (2 * valvenum + 2) +
        ...
        // threedmux_control.addChild(circle);
        let rightcontrol = new paper.Path.Rectangle(
          rightsideLeft,
          rightsideRight
        );
